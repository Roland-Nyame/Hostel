...
const server = http.createServer((request, response) => {
  ...
  if (request.method === 'POST' && url.pathname === '/api/rooms/reset') {
    if (user.role !== 'admin') return send(response, 403, { error: 'Only Admin users can reset rooms' });

    // Reset occupancy only: keep every room's ID/capacity, but make every room available.
    // All current bed allocations and student assignments are cleared.
    ...
    persist();
    return send(response, 200, { success: true, rooms });
  }
